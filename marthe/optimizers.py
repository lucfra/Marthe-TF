...
class OptimizerDict(object):
    def __init__(self, ts, dynamics, objective, global_step=None, dynamics_func=None):
        self._ts = ts
    ...
    def dynamics_func(self, loss, variables):
        if self._dynamics_func:
            return self._dynamics_func(loss, variables)
        else:
            raise AttributeError('Dynamics function not yet available')
...
class Optimizer(tf.train.Optimizer):
    def minimize(self, loss, global_step=None, var_list=None, gate_gradients=tf.train.Optimizer.GATE_OP,
        ...
        return OptimizerDict(ts=ts, dynamics=dyn, objective=loss, global_step=global_step)
...
class GradientDescentOptimizer(Optimizer, tf.train.GradientDescentOptimizer):
    ...
    def minimize(self, loss, global_step=None, var_list=None, gate_gradients=Optimizer.tf().GATE_OP,
                 aggregation_method=None, colocate_gradients_with_ops=False, name=None, grad_loss=None):
        ts, dyn = super(Optimizer, self).minimize(loss, global_step, var_list, gate_gradients, aggregation_method,
                                                  colocate_gradients_with_ops, name, grad_loss)
        return OptimizerDict(ts=ts, dynamics=dyn, objective=loss, global_step=global_step,
                             dynamics_func=lambda _loss, _vars:
                             [v - tf.cast(self._learning_rate_tensor, g.dtype)*g for
                              v, g in zip(_vars, tf.gradients(_loss(_vars), _vars))]
                             # of course for other optimization methods this may become a bit more tricky
                             )
